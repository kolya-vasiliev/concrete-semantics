...
theory Small_Step_7_9 imports Star Big_Step_7_9 begin
...
Or1:     "(c\<^sub>1 OR c\<^sub>2,s) \<rightarrow> (c\<^sub>1,s)" |
Or2:     "(c\<^sub>1 OR c\<^sub>2,s) \<rightarrow> (c\<^sub>2,s)" 
...
next
  fix b t c s
  let ?ru = "IF b THEN SKIP ELSE REPEAT c UNTIL b"
  assume "bval b t"
  hence "(?ru, t) \<rightarrow>* (SKIP, t)" by auto
  moreover assume  "(c, s) \<rightarrow>* (SKIP, t)"
  ultimately have "(c;; ?ru, s) \<rightarrow>* (SKIP, t)" using seq_comp by blast
  moreover have "(REPEAT c UNTIL b, s) \<rightarrow> (c;; ?ru, s)" by auto
  ultimately show "(REPEAT c UNTIL b, s) \<rightarrow>* (SKIP, t)" by (auto intro: star.step)
next
  fix b s\<^sub>2 c s\<^sub>1 s\<^sub>3
  let ?ru = "IF b THEN SKIP ELSE REPEAT c UNTIL b"
  assume "\<not> bval b s\<^sub>2"
  assume "(c, s\<^sub>1) \<rightarrow>* (SKIP, s\<^sub>2)"
  hence "(REPEAT c UNTIL b, s\<^sub>1) \<rightarrow>* (SKIP;; ?ru, s\<^sub>2)" using star_seq2 
    by (auto intro: star.step)
  moreover have "(SKIP;; ?ru, s\<^sub>2) \<rightarrow>* (REPEAT c UNTIL b, s\<^sub>2)" using `\<not> bval b s\<^sub>2` 
    by (auto intro: star.step)
  moreover assume "(REPEAT c UNTIL b, s\<^sub>2) \<rightarrow>* (SKIP, s\<^sub>3)"
  ultimately show "(REPEAT c UNTIL b, s\<^sub>1) \<rightarrow>* (SKIP, s\<^sub>3)" 
    by (auto intro: star_trans)
next
  fix c1 s t c2
  assume "(c1, s) \<rightarrow>* (SKIP, t)"
  moreover have "(c1 OR c2, s) \<rightarrow> (c1, s)" by simp
  ultimately show "(c1 OR c2, s) \<rightarrow>* (SKIP, t)" by (metis star.step)
next
  fix c1 s t c2
  assume "(c2, s) \<rightarrow>* (SKIP, t)"
  moreover have "(c1 OR c2, s) \<rightarrow> (c2, s)" by simp
  ultimately show "(c1 OR c2, s) \<rightarrow>* (SKIP, t)" by (metis star.step)
qed
...
next
  case Or1
  thus ?case by (metis small_step.Or1 star.step)
next
  case Or2
  thus ?case by (metis small_step.Or2 star.step)  
qed
